...
class AttentionRNN(object):
    def __init__(self, vocabulary_size, document_max_len, num_class):
        ...
        with tf.name_scope("output"):
            self.logits = tf.layers.dense(self.attention_out, num_class, activation=None, name="logits")
            self.predictions = tf.argmax(self.logits, -1, output_type=tf.int32, name="prediction")
